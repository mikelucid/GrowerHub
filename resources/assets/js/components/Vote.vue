<template>
  <div class="d-flex flex-column-reverse flex-grow-0 align-items-center votings ml-1 mr-3">
    <a @click.prevent="voteUp" :title="title('up')" class="vote-up" :class="classes">
      <font-awesome-icon icon="caret-down" class="fa-2x hit-voting" />
    </a>
    <span><favorite v-if="name === 'question'" :question="model"></favorite>
      <accept v-else :answer="model"></accept></span>
    <a @click.prevent="voteDown" :title="title('down')" class="vote-down" :class="classes">
      <font-awesome-icon icon="caret-up" class="fa-2x hit-voting" />
    </a>
